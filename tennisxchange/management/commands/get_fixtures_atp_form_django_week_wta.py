...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        today = datetime.now()
        one_week_ago = today - timedelta(days=7)  # Changed from yesterday to one_week_ago
        date_stop = today.strftime('%Y-%m-%d')
        date_start = one_week_ago.strftime('%Y-%m-%d')  # Changed from yesterday to one_week_ago


        event_type_keys = ["266"]  # WTA Singles
        fixtures_data = {}
